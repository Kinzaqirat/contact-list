...
choices:["Add contact","Veiw contact","Close menu"]
    ...
    if(askInput.ask==="Add contact")addContact();
     if (askInput.ask==="Veiw contact")veiwContact()   
     if(askInput.ask==="Close menu")closeMenu();
}
main();
...
if (list.length >0)list.forEach((user)=>
    console.log(`\n ${user.ID}. Person Name: ${user.name} ---- Contact Number: ${user.phoneNumber}`))
else{console.log("No Contact available!");}
main()
